...
async function run() {
  try {
    ...
    app.patch("/payment-success", async (req, res) => {
      ...
      // If already processed, return full invoice data from stored payment + booking
      const paymentExist = await paymentCollection.findOne({ transactionId });
      if (paymentExist) {
        const booking = await bookingsCollection.findOne({
          _id: new ObjectId(paymentExist.bookingId),
        });
        return res.send({
          invoice: {
            transactionId: paymentExist.transactionId,
            trackingId: paymentExist.trackingId,
            bookingId: paymentExist.bookingId,
            serviceName: paymentExist.serviceName,
            amount: paymentExist.amount,
            currency: paymentExist.currency,
            customerEmail: paymentExist.customerEmail,
            customerName: paymentExist.customerName || booking?.userName || "",
            paidAt: paymentExist.paidAt,
            eventDate: booking?.eventDate || "",
            region: booking?.region || "",
            district: booking?.district || "",
            address: booking?.location?.addressLine || "",
          },
        });
      }
      ...
      await bookingsCollection.updateOne(bookingQuery, {
        $set: { paymentStatus: "paid", BookingStatus: "confirmed", trackingId },
      });
